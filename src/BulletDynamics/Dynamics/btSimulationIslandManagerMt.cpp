...
// avoid solving an island in parallel if the manifold count is this large or more
int gLargeIslandManifoldCount = 1000;
...
void btSimulationIslandManagerMt::parallelIslandDispatch( btAlignedObjectArray<Island*>* islandsPtr, IslandCallback* callback )
{
    ...
    while (iBegin < islandsPtr->size())
    {
        ...
        if (island->manifoldArray.size() < gLargeIslandManifoldCount )
        {
